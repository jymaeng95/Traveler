...
<tabtable>
  ...
  <label for="tab01" class="tab"><a href="rcv_list">받은 쪽지함</a></label>  
  ...
  <label for="tab02" class="tab"><a href="send_list">보낸 쪽지함</a></label>
  ...
  <label for="tab03" class="tab"><a href="archive">보관함</a></label>
    ...
    <table class="table table-striped table-hover" style="table-layout:fixed">
      ...
      <c:forEach items="${msg_list}" var="list" varStatus="status">
        <tr>
          ...
          <td>${list.sender}<input id="parentId" value="${list.sender}" style="display:none"></td>
          <td style="text-overflow:ellipsis; overflow:hidden; white-space:nowrap;"><a type="button" onclick="updateMsg2('${userInfo.userId}','${list.mid}','1');" >
          ${list.mcontent}</a><input id="parentId3" value="${list.mcontent}" style="display:none"></td>
          <td>
          <p id="date${status.index }">${list.senddate }</p><input id="parentId2" value="${list.senddate}" style="display:none">
          </td>
    ...
    <div class='row text-center'>
    	<ul class="pagination">
    		...
    		<c:forEach var="num" begin="${pageMaker.startPage }" end="${pageMaker.endPage }">
    			<li class="paginate_button ${pageMaker.pageNum == num ? "active":"" }"><a href="${num }">${num }</a>
    		</c:forEach>
    ...
    <table class="table table-striped table-hover" style="table-layout:fixed">
      ...
      <c:forEach items="${msg_list2}" var="list" varStatus="status">
        <tr>
          ...
          <td style="text-overflow:ellipsis; overflow:hidden; white-space:nowrap;"><a type="button" onclick="popup2();" >
          ${list.mcontent}</a><input id="parentId3" value="${list.mcontent}" style="display:none"></td>
          <td>
          <p id="date${status.index }">${list.senddate }</p><input id="parentId2" value="${list.senddate}" style="display:none">
          </td>
    ...
    <div class='row text-center'>
    	<ul class="pagination">
    		<c:if test="${pageMaker2.prev }">
    			<li class="paginate_button previous"><a href="${pageMaker2.startPage -1}">Previous</a></li>    			
    		</c:if>
    		<c:forEach var="num" begin="${pageMaker2.startPage }" end="${pageMaker2.endPage }">
    			<li class="paginate_button ${pageMaker2.pageNum == num ? "active":"" }"><a href="${num }">${num }</a>
    		</c:forEach>
    ...
    <div class='row text-center'>
    	<ul class="pagination">
    		<c:if test="${pageMaker3.prev }">
    			<li class="paginate_button previous"><a href="${pageMaker3.startPage -1}">Previous</a></li>    			
    		</c:if>
    		<c:forEach var="num" begin="${pageMaker3.startPage }" end="${pageMaker3.endPage }">
    			<li class="paginate_button ${pageMaker3.pageNum == num ? "active":"" }"><a href="${num }">${num }</a>
    		</c:forEach>
    		<c:if test="${pageMaker3.next }">
    			<li class="paginate_button next"><a href="${pageMaker3.endPage +1 }">Next</a>
    		</c:if>
    	</ul>
    	<form id='actionForm' action="/mypage/message" method='get'>
    ...
    <div class='row text-center'>
    	<ul class="pagination">
    		<c:if test="${pageMaker4.prev }">
    			<li class="paginate_button previous"><a href="${pageMaker4.startPage -1}">Previous</a></li>    			
    		</c:if>
    		<c:forEach var="num" begin="${pageMaker4.startPage }" end="${pageMaker4.endPage }">
    			<li class="paginate_button ${pageMaker4.pageNum == num ? "active":"" }"><a href="${num }">${num }</a>
    		</c:forEach>
    		<c:if test="${pageMaker4.next }">
    			<li class="paginate_button next"><a href="${pageMaker4.endPage +1 }">Next</a>
    		</c:if>
    	</ul>
    	<form id='actionForm' action="/mypage/message" method='get'>
