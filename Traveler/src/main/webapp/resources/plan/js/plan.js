...
//				$("#r_mapY"+key).text(data[key].mapY);
				...
				reco_positions.push({title : data[key].title, lonlat : new Tmapv2.LatLng(data[key].mapX, data[key].mapY), addr : data[key].addr1 , overview : data[key].overview, img :  data[key].firstImage2 });
...
function addMarkers(map,positions){
	...
	for (var i = 0; i< positions.length; i++){//for문을 통하여 배열 안에 있는 값을 마커 생성
		...
		var overview = positions[i].overview;
		var img = positions[i].img;
		//Marker 객체 생성.
		...
		markerClick(map,marker,lonlat,title,addr,overview,img);
		markers.push(marker);
	}
	return markers;
}
...
function markerClick(map,marker,latlng,title,addr,overview,img){
	marker.addListener("click",function(evt){
		...
		"<p class='new-addr'><i class='fas fa-map-marker-alt' style='padding-right:3%'></i>"+addr+"</p>"+
/*		"<p class='old-addr' style='color: #707070;'>(우)04539 (지번)을지로 2가 11</p>"+*/
		...
		"<i class='far fa-sticky-note' style='padding-right:3%'></i>"+overview+"</p>"+
		"</li>"+
		"</ul>"+
		...
		"<a href='#' title='더 보기'><i class='fas fa-1x fa-info-circle'></i></a>"+
		"</li>"+
		"<li style='display: table-cell; vertical-align: middle; width: 50%; height: 100%; border-right: 1px solid #EFEFEF;''>"+
	...
	$.ajax({
		...
		success : function(data) {
			$.each(data, function(key, val){
				...
				positions.push({title : data[key].title, lonlat : new Tmapv2.LatLng(data[key].mapX, data[key].mapY),addr : data[key].addr , overview : data[key].overview, img :  data[key].img_src});
//				$("#addr"+key).text(data[key].addr1);
//				$("#overview"+key).html(data[key].overview);
//				$("#btn-detail"+key).attr("onclick","imgClick("+data[key].contentId+","+data[key].contentTypeId+");");
				$(".totalCount").text(data[key].totalCount);
